...
def atomic(path,data):
 ...
 os.replace(tmp,path)
def main():
 p=argparse.ArgumentParser();p.add_argument('--resume',action='store_true');p.add_argument('--checkpoint',type=Path,required=True);p.add_argument('--policy',choices=POLICIES);p.add_argument('--start-episode',type=int,default=1);p.add_argument('--episode-count',type=int,default=20);p.add_argument('--seed',type=int,default=20260722);p.add_argument('--output-dir',type=Path,default=ROOT/'runs/final-evaluation/rl-docker-supplement');a=p.parse_args()
